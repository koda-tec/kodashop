  ...
  return (
    <>
      ...
      <button 
        ...
        className="fixed top-4 left-4 z-60 md:hidden bg-blue-600 p-2 rounded-lg text-white"
      >
        {isOpen ? <X size={24}/> : <Menu size={24}/>}
      </button>

      {/* Overlay para cerrar al tocar fuera en mobile */}
      {isOpen && (
        <div 
      ...
      <aside className={`
        fixed md:sticky top-0 left-0 h-screen w-72 bg-slate-900 border-r border-slate-800 p-8 z-50
      ...
      `}>
        ...
        <nav className="flex flex-col gap-6">
          {["Catálogo", "Conversión"].map(group => (
            <div key={group}>
              <p className="text-[10px] font-black text-slate-500 mb-3 tracking-[0.2em] uppercase px-4">{group}</p>
              <div className="flex flex-col gap-1">
                {menuItems.filter(item => item.group === group).map(item => (
                  <button
                    key={item.id}
                    onClick={() => { setActiveTab(item.id); setIsOpen(false); }}
                    className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === item.id ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/20' : 'text-slate-500 hover:bg-slate-800'}`}
                  >
        ...
        <button 
          ...
          className="mt-auto flex items-center gap-3 text-slate-500 hover:text-white transition font-black text-[10px] uppercase tracking-widest border-t border-slate-800 pt-6 w-full"
        >
